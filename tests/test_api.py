...
class APITestCase(unittest.TestCase):
    ...
    def test_bad_auth(self):
        ...
        u = User(email='john@localhost.loc', password='cat', confirmed=True,
                 role=r)
        db.session.add(u)
        db.session.commit()
        ...
        response = self.client.get(
            '/api/v1/posts/',
            headers=self.get_api_headers('john@localhost.loc', 'dog'))
        self.assertEqual(response.status_code, 401)
    ...
    def test_token_auth(self):
        ...
        response = self.client.post(
            '/api/v1/tokens/',
            headers=self.get_api_headers('john@localhost.loc', 'cat'))
    ...
    def test_unconfirmed_account(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u = User(email='john@localhost.loc', password='cat', confirmed=False,
                 role=r)
        db.session.add(u)
        db.session.commit()
    ...
    def test_posts(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u = User(email='john@localhost.loc', password='cat', confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()
    ...
    def test_users(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u1 = User(email='john@localhost.loc', username='john',
                  password='cat', confirmed=True, role=r)
        u2 = User(email='susan@localhost.loc', username='susan',
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers('susan@localhost.loc', 'dog'))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
    ...
    def test_comments(self):
        ...
        response = self.client.post(
            ...
            headers=self.get_api_headers('susan@localhost.loc', 'dog'),
            data=json.dumps({'body': 'Good [post](http://localhost.loc)!'}))
        self.assertEqual(response.status_code, 201)
        json_response = json.loads(response.get_data(as_text=True))
        url = response.headers.get('Location')
        self.assertIsNotNone(url)
        self.assertEqual(json_response['body'],
                        'Good [post](http://localhost.loc)!')
        self.assertEqual(
